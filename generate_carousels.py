    ...
    for library in config:
        if library.startswith("lib-"):
            if library != "lib-consortium":
                url = (
                    "http://"
                    + return_lib_subdomain(library)
                    + "."
                    + get_host()
                    + "/opac/extras/browse/atom/item-age/"
                    + return_lib_shortname(library)
                    + "/1/"
                    + item_return_count(library)
                    + get_statuses(library)
                    + get_copy_locations(library)
                )
            else:
                url = (
                    "http://"
        ...
        if library != "None":
            for tcn in tcn_list:
                image_url = (
                    "http://"
                    + library
                    + ".cool-cat.org/opac/extras/ac/jacket/medium/r/"
                    + tcn
                )
        ...
        with open(filename, "w") as file:
            ...
            if library == "None":
                for tcn in good_tcns:
                    file.write(
                        '<div><a href="https://cool-cat.org/eg/opac/record/'
                        + tcn
                        + '" target="_blank"><img src="https://cool-cat.org/opac/extras/ac/jacket/medium/r/'
                        + tcn
                        + '"></a></div>'
                    )
            else:
